...
| `coffee_backtests/06_composite_backtest.ipynb` | Full composite: aligns all 5 signals onto one monthly frame (reusing nb04's step-function forward-fill for L2a/L3), derives weights via r-proportional reweighting (vs Phase 0 fixed weights and a Ridge ceiling check), runs a leave-one-out ablation, walk-forward gates on `≥3.50%` forward prescience. **PASSES both weight schemes** (Phase 0 +9.31%, r-proportional +3.62%) but on a thin walk-forward sample (11-21 buy-months across 10 years) — treat as directional, not final. Ablation: L2b strongest contributor, L5 helps (validates momentum role), L3 near-neutral, L2a counterintuitive (needs its risk score rebuilt off the true-vintage series before finalizing) | ✅ Done |
