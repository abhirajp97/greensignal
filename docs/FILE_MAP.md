...
| `usda_coffee_wmt.py` | USDA "Coffee: World Markets and Trade" semiannual PDF circular (`esmis.nal.usda.gov`, free, no auth) — true vintage-dated world stocks-to-use %, fixes `usda_psd.py`'s look-ahead bias. Not WASDE (WASDE doesn't cover coffee). Also exports `stu_wmt_zscore()`/`stu_wmt_stress_score()` — the true-vintage, non-linear stress score (notebook 04 §16), promoted here from notebook-local status; feeds both the Brazil composite's `stu_risk` weight and India v3's `stu_stress` input. Deps: `pdfplumber`, `beautifulsoup4` | **6th** — free, no auth | ✅ Done |
...
| File | What it does | Status |
|------|-------------|--------|
| `signal_generator.py` | `generate_signal()` — Brazil, takes all five signal inputs → calls `recommendation_engine.build_recommendation`. `generate_india_signal()` — v2, India's 2-input (price_position + Kodagu climate risk) composite, same conditional-multiplicative formula shape as Brazil; **superseded** by `generate_india_signal_v3()` (`INDIA_V3_WEIGHTS`) — India-specific, walk-forward-validated (notebook 09 §16) additive weighted-sum of momentum-signed `price_position` + true-vintage `stu_stress` + Brazil's own drought risk (as a global Arabica proxy, not Kodagu-local) | ✅ Done |
...
| `coffee_backtests/06_composite_backtest.ipynb` | Full composite: aligns all 5 signals onto one monthly frame (reusing nb04's step-function forward-fill for L2a/L3). **Rebuilt** from a discrete rare-event "prescience" gate (thin sample — 11-21 buy-months/10yrs, 3-4 silent years) to match the product's actual continuous always-on design: rolling-24m normalization (vs the old annual-refit, which was the sparsity's root cause — 20.0% vs 4.9% BUY rate) + `cost_improvement_backtest` (continuous volume-scaling) as the primary gate. **PASSES**: walk-forward +5.86%, beating L1-alone's own +3.71% benchmark. Confirms spike-avoidance story (sustained BUY signal before the 2024 rally). §11-§12: re-ran the L2a ablation on the true-vintage `stu_stress` (nb04 §16) — wrong-signed, dropping it improves the ablation more than the approximation did; final weights re-derived and promoted into `climate_features.py::WEIGHTS`. §13: cross-validates `core.services.recommendation_engine.classify_normalized()` against this notebook's own rolling computation (matches to 4e-16) and saves `data/06_composite_history_true.csv` — Brazil's real historical composite history, consumed by notebook 09 §10's transferability test. **§14-§16 (new): both §7/§8 anomalies investigated further.** §14: re-ran §7's 3-state test with true walk-forward weights (not fixed) — anomaly gets *worse*, not resolved (CAUTION +20.26% fwd-6m vs BUY +7.47%), falsifying the in-sample-weights hypothesis; directly confirms the structural-rally hypothesis instead (8/19 CAUTION months fall in the 2021/2024 rallies). §15: traces the L5 ablation sign reversal to `cot_momentum`'s real, pre-documented sign instability (rolling-36m r: 52% positive, mean≈0) meeting the composite's sign-stability-assuming weighting scheme — no weight change follows, but the mechanism is now verified, not just asserted. §16: summary | ✅ Done |
...
| `coffee_backtests/09_india_origin_signal.ipynb` | India origin backtest v2 (Arabica + Robusta, Karnataka) — rebuilt entirely on REAL India-origin data: price from `coffee_board_india_price.py` (9,217 obs, 2014-2026), supply from `coffee_board_india_supply.py`, climate from `chirps_india.py` (species-specific SPI-3 windows, now multi-district). Gate (annual r ≥ +0.30): Robusta r=−0.357 **FAIL**, Arabica r=−0.038 **FAIL** — both closed out via 2 negative experiments (multi-district weighting, Arabica 24m robustness). §6-7: global pass-through + FX decomposition — `log(india_price) ~ log(global_price) + log(usd_inr)` gives **R²=0.887 (Robusta) / 0.962 (Arabica)**, explaining 89-96% of India's price; residual re-test against the same climate gate still FAILS both species. §10: the §15 forecast-transferability test — does the Brazil composite's real regime predict *forward* India price, not just the contemporaneous level relationship? **FAIL, decisively**: r(Brazil `normalized_mult`, fwd-12m India Arabica price)=−0.250 (p=0.005, n=127), wrong-signed; FX-adjusted version nearly identical (r=−0.235) — not an FX-noise artifact. `price_position_52w` sanity check passes cleanly. Composite (§8) wired for demo purposes (v2, `generate_india_signal`), confidence=0.3. **§11-§16: diagnosed the §15 FAIL and found a real signal.** §11-§12 trace the FAIL to Brazil's `(1.5 - price_position)` contrarian term being wrong-signed for India even though 4/5 raw sub-signals (incl. `l3_risk`, Brazil's own drought risk as a global-supply proxy, r=+0.620 @ 12m) are right-signed. §13-§15 re-derive weights on India's own data: `enso_risk`/`cot_momentum` dropped (contribute ~nothing or hurt); best candidate `price_pos` (momentum-signed, 0.361) + `stu_stress` (0.148) + `l3_risk` (0.491), additive not multiplicative. §16: **walk-forward validated, r=+0.553 (p<0.0001, n=96, 2017-2024)** — now productionized as `generate_india_signal_v3` in `signal_generator.py` | ✅ Done (v2 gate FAILs, real data, fully diagnosed; v3 signal found and productionized — see notebook §9-§16) |
