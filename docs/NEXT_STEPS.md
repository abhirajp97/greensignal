...
- [x] **Run `notebooks/coffee_backtests/08_producer_fx_signal.ipynb`** — producer FX PoC (USD/BRL → KC). G1 PASS (monthly r=−0.316); G2 FAIL (0 controlled survivors after DXY/KC controls); G3 FAIL (deferral saving −0.14%). Co-movement is real but not an exploitable timing lead — treat as conditioning/regime variable, not a composite timing input. FX index did not clearly beat BRL alone
