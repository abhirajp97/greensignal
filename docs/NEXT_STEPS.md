...
- [ ] **Deferred: build a true WASDE-vintage S/U series** — parse ~25 years of monthly WASDE PDF/HTML reports to reconstruct the actual point-in-time series (the 12m shift is an approximation, not a literal vintage archive). Scoped as its own `implement-source` effort; only prioritize if L2a's composite weight (notebook 06) turns out sensitive to the extra precision
- [ ] **Promote `stu_z_score` / `stu_stress_score`** from notebook 04 into `domains/coffee/features/supply_features.py` (currently a stub) when wiring the composite — mirrors the SPI-promotion item below for L3
...
- [ ] **Run `notebooks/coffee_backtests/06_composite_backtest.ipynb`** — all 5 sources now implemented. Re-validate the composite on real data with the revised signal roles (L1 strong; L2a strong on level; **L2b now a validated ~14m El Niño lead, r≈+0.29–0.33**; **L3 now a validated confirming amplifier via the SPI flowering deficit, annual r=+0.48**; L5 the lone failed signal). When wiring L2b, apply a **~14m lead** and the **flipped** `enso_risk_score` (El Niño→high risk); wire L3 as the SPI flowering-deficit z-score. Confirm/adjust the conditional formula and weights
...
| L2a vintage-lag fix uses a 12m shift, not a WASDE archive | Practical fix given USDA doesn't publish a bulk vintage-history file; a true point-in-time series needs its own monthly WASDE-parsing source — deferred until the composite shows it's load-bearing |
...
*Update: 2026-07-13 — **rebuilt L2a (USDA stocks-to-use) around a vintage-lag correction** after review feedback flagged the original notebook's use of the PSD bulk file (always the latest-*revised* vintage) as a look-ahead bias. Shifted the monthly S/U series forward 12 months to simulate the real publication lag (raw r(S/U, price level)=−0.40 weakens to r=−0.26 once corrected — the honest number). Redefined the single YoY-change gate into two tests on the vintage-lagged series, both **PASS**: Gate 1 r(vintage S/U, 12m-fwd price)=−0.312 (p=2.0e-5, n=180), Gate 2 r(S/U YoY delta, price level)=−0.259 (p=4.6e-4, n=180). Added a 10yr rolling z-score (no additional look-ahead), YoY delta, and months-of-consumption features, plus a z-score-based non-linear stress score replacing the linear 12%/35% clamp. Rolling 3yr stability reported honestly as modest (52% of windows negative — real improvement over 23% on the old YoY basis, but not strong on its own; the full-sample gates carry the finding). Declined to build a full WASDE-vintage archive (out of scope — its own data-source effort) and declined to promote the new features into production `supply_features.py` yet (per the L3/SPI precedent, wait for composite wiring in notebook 06). `make verify` not yet re-run this session — do so before merging. Next: composite backtest (06).*
