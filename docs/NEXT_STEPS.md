...
- [ ] **Decide L5 fate** — flip to a 3–6m momentum-confirmation role (use +COT index, low weight) or drop L5 from the composite. Revisit sign/threshold of `cot_contrarian_signal` before wiring into `signal_generator`. **New evidence leans toward "drop as standalone":** the canonical walk-forward test loses −2.26% avg vs naive (1/12 positive years)
- [x] **Add walk-forward purchase simulations to notebooks 01 (re-executed), 02, 03, 05** — same naive-vs-weighted-average design across all four; see `CHANGELOG.md [0.16.0]` for full results. Headline: L1 is the only signal that clears both its correlation gate *and* a walk-forward economic bar (+7.34% avg, 10/12 positive, 78w window — corrected from a stale/unreproducible "+3.71%, 12/12" figure that was never actually in the executed notebook). ENSO, CHIRPS, and COT all fail to show a reliable standalone economic edge regardless of whether their correlation gate passes or fails
- [x] **Write/update reader's guides for all five backtest notebooks** — `docs/notebook_0{1..5}_guide.md`, matching the existing 01/03 format
- [ ] **Add a walk-forward purchase simulation to notebook 04 (L2a)** — the only one of the five without one yet. Given L2a's much stronger price-level correlation (r=−0.40 to −0.59) than ENSO/CHIRPS/COT, it's a reasonable hypothesis this would fare better in a walk-forward test, but that's untested
...
*Last updated: 2026-08-04 — **added walk-forward purchase simulations to notebooks 01 (re-executed), 02, 03, and 05**, and wrote/updated reader's guides for all five backtest notebooks. Notebook 01's own walk-forward cells had never actually been executed in the committed file — the "+3.71% avg, 12/12 years" figure cited in CLAUDE.md and the notebook 01 guide was not reproducible; re-execution shows the 78w window (not 104w) wins at +7.34% avg / 10/12 positive years. Applied the same naive-vs-weighted-average design to ENSO (Sec 5 in notebook 02, replacing the old gate/event-study/risk-distribution sections), COT (Sec 7 in notebook 03, replacing the old rolling-stability section, alongside the notebook's existing multi-variant walk-forward in Sec 5), and CHIRPS (Sec 6 in notebook 05, replacing an old rolling-stability section that was found to test the wrong lag). Consistent finding across all three: passing or failing the correlation gate does not predict the walk-forward result — only L1 clears both. Notebook 04 (L2a) does not have a walk-forward section yet. See `CHANGELOG.md [0.16.0]` for full numbers.*

*Prior update: 2026-06-08 — **corrected the L2b ENSO thesis.** The original "La Niña droughts Brazil/Vietnam, 24m lag" had the sign AND lag backwards. Verified the country matrix (`docs/enso_coffee_country_matrix.html`) against peer-reviewed sources: El Niño is the dominant supply-risk phase (droughts Vietnam/Indonesia robusta; La Niña is beneficial there & in Brazil, only hurts Colombia). Flipped `enso_risk_score` in `noaa_enso.py` and rebuilt notebook 02 — redefined gate (r ≥ +0.20 vs fwd YoY at ~14m lead) now **PASSES**: r=+0.288 @15m (KC) / +0.327 @15m (WB 2000–24); event study El Niño→+36.5% vs La Niña −1.7%. 26 ENSO tests updated & green. Next: composite backtest (notebook 06) — L1 strong, L2a strong-on-level, L2b now a validated ~14m lead, L3/L5 weak amplifiers; then redefine L2a gate, decide L5 fate.*
