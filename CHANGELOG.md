...
- Production wiring for the Brazil composite: `climate_risk_score` weights, `risk_scorer.py`, promote `stu_risk_score` / `enso_lagged` / the SPI drought score into production feature files
...
### Findings (notebook 09 §4c/§4d — both experiments flagged in [0.21.0], now closed out)
...
- **Experiment 2, robustness check on the Arabica 24m hint — the hint does not survive scrutiny.** Leave-one-out on the [0.21.0] r=+0.179 @ 24m figure ranges **[−0.065, +0.395]** — the sign itself flips depending on which single year is excluded. A 5,000-resample bootstrap 90% CI is **[−0.356, +0.622]** — spans both zero and the +0.30 gate. Conclusion: the 24m hint was noise, not a real weak effect — this resolves the ambiguity [0.21.0] left open ("worth revisiting"); only genuinely new years of price history, not cleverer analysis of the same 12 points, can move this.
...
### Added
- **Real India-origin price and supply sources, superseding the WB-proxy fallback from [0.20.0].** Told explicitly not to settle for a proxy and not to self-impose artificial timelines, a second pass found that `indiacoffee.org` ([0.20.0]'s dead end) was simply the wrong domain — Coffee Board of India's actual live site is **`coffeeboard.gov.in`**.
...
- **Gate (annual r ≥ +0.30) still FAILS for both species — but now a diagnosed result, not a proxy-limitation excuse.** Robusta: r=−0.357 at 12m (n=12) — wrong-signed at every tested horizon from 3 to 24 months, not a borderline miss. Arabica: r=−0.038 at 12m (n=12) — much closer to zero than [0.20.0]'s proxy-based −0.214, and turns positive at longer horizons (24m: +0.179, echoing L1's own 24m mean-reversion finding), but still short of the bar. Supply (production YoY) vs forward-6m price: r=−0.110, p=0.564, n=30 — no standalone relationship.
...
- Composite (`generate_india_signal()`) still runs live for demo purposes; confidence lowered from [0.20.0]'s 0.4 to 0.3, since this is now a tested-and-failed mechanism, not merely an untested proxy.

### Why it matters
The India origin signal now has real, genuinely India-origin data end to end — a meaningfully stronger foundation than [0.20.0]'s proxy, even though the gate still fails. The failure is now informative rather than a known limitation to excuse: it rules out (at this sample size) the specific hypothesis that Kodagu blossom-window rainfall predicts forward India coffee price, while confirming the rest of the pipeline (price data, `price_position_52w`) works correctly. This sets up two concrete next experiments — a production-weighted multi-district climate signal, and retesting the Arabica 24m hint as more history accumulates — rather than leaving "the price is a proxy" as an open, unfalsifiable excuse.
