...
- Confirm which composite formula (of the several under parallel exploration) is being wired first for Brazil production
- Card-copy calibrated-honesty review for India, now incorporating §16's real walk-forward-validated signal (`generate_india_signal_v3`) alongside §14's pass-through finding and §15's Brazil-regime-transfer FAIL — not yet promoted to roaster-facing "validated" framing (single walk-forward test, Arabica-only)
...
- India origin signal: full 62-report supply re-backfill, blocked on `coffeeboard.gov.in` renewing its expired TLS certificate; discrete regulatory/policy event dummies (EUDR, India-EFTA) as a fundamentally different feature type; harvest-window (Nov-Feb) rainfall as a different climate pathway from the flowering-window SPI-3 already built; Robusta re-test of the v3 signal (§16 is Arabica-only)

---

## [0.26.0] — 2026-08-05

### Investigated — notebook 06's two open anomalies, both taken further rather than left as noted discrepancies
- **§14: re-tested the §7 BUY-underperforms-CAUTION anomaly with genuinely out-of-sample, walk-forward-refit weights** (same machinery as §6/§8) instead of §7's fixed full-history weights — the tempering hypothesis this was meant to test. **Result: falsified, not confirmed — the anomaly is *worse* with true walk-forward weights.** CAUTION's forward-6m return rose to **+20.26%** (n=19) against BUY's +7.47% (n=12), a wider gap than the original fixed-weight version (CAUTION +6.99% vs BUY +4.66%). Directly inspecting the CAUTION months confirms the notebook's *second* tempering hypothesis instead: 8 of 19 fall in Dec 2020–Jun 2021 or Jun–Dec 2024 — the sample's two genuine structural supply-shock rallies — where the composite correctly read price as elevated relative to its own recent regime, but the rally kept running well past that point (forward-6m returns of +41% to +53% in several of those months). The primary economic gate (§6, cost-improvement, which passes) remains the metric that should anchor the product decision; the 3-state classification's forward-return ordering should not be marketed as reliably monotone without this caveat.
- **§15: traced the §8 L5 (`cot_momentum`) ablation sign reversal to a specific, verified mechanism** rather than leaving it as "sensitive to metric and weighting scheme." Every walk-forward training window's `r(cot_momentum, fwd_6m)` on the final true-vintage frame (2013–2024) is *negative* (−0.026 to −0.541) — opposite notebook 03's own full-sample 2010–2024 finding (+0.144) — while a rolling-36-month stability check on the full frame shows the correlation is close to a coin flip (52% of windows positive, mean ≈ 0.00, range −0.560 to +0.395), consistent with and slightly more extreme than notebook 03's own 45%-positive figure for the same signal. The composite's r-proportional weighting scheme (weight ∝ `abs(r)`, but the raw non-negative sub-signal value is what actually gets added) implicitly assumes each sub-signal stays consistently right-signed; when a training window's own `r` flips negative, weighting `cot_momentum` up by magnitude while adding it as if higher = higher-risk is mechanically backwards for that window — sufficient to explain the sign flip. **No weight change follows** — `cot_momentum` stays at its existing 0.031 weight; this traces the existing number to a mechanism instead of proposing a new one.
- Both investigations are new sections (§14–§16) appended to `notebooks/coffee_backtests/06_composite_backtest.ipynb`, executed end-to-end (not hand-derived) and cross-checked against the notebook's own already-cached numbers where they overlap.

### Added — true-vintage `stu_stress` promoted from notebook-local to production
- **`domains/coffee/sources/usda_coffee_wmt.py::stu_wmt_zscore()` / `stu_wmt_stress_score()`** — the non-linear stress score (`clamp((-z + 2) / 4, 0, 1)`, window=6 semiannual reports) notebook 04 §16 built and notebook 06 §11–§12 promoted into `climate_features.py::WEIGHTS`, but which stayed notebook-local until now. Byte-for-byte cross-checked against notebook 04's cached `stu_wmt_vintage.csv` values (0 mismatches across 25 valid observations) — caught and fixed one real bug in the process: an initial population-stdev (`statistics.pstdev`) implementation was consistently off by a factor of `sqrt(6/5)` from pandas' sample-stdev (`ddof=1`) default the notebook actually used. 5 new tests.

### Added — India v3 signal, productionized
- **`domains/coffee/models/signal_generator.py::generate_india_signal_v3()`** (`INDIA_V3_WEIGHTS`) — notebook 09 §16's walk-forward-validated India Arabica timing signal (r=+0.553, p<0.0001, n=96, 2017-2024), no longer notebook-only. Two structural differences from every other formula in the product, both load-bearing findings from notebook 09 §11-§12, not stylistic choices: (1) `price_position` is fed in **momentum-signed** (raw, not `1.5 - x`) — India's price position correlates *positively* with forward India price at 3-12m horizons, the opposite of the mean-reversion assumption validated for global Arabica (notebook 01); (2) the formula is a **plain additive weighted sum**, not Brazil's conditional-multiplicative shape — the multiplicative structure lets one wrong-signed term (the contrarian price term) dominate the product's overall sign, which is precisely why §15's straight test of Brazil's own formula FAILed. `l3_risk` is deliberately **Brazil's own** Minas Gerais drought risk, reused as a global Arabica supply-risk proxy (Brazil dominates world production) — not Kodagu/India-local climate data, consistent with §6-7's pass-through finding that local India climate isn't the right mechanism. `stu_stress` uses the newly-promoted `usda_coffee_wmt.stu_wmt_stress_score()`. `confidence` defaults to 0.4 (lower than Brazil's typical 0.7) — one walk-forward test on ~8 years of India-specific data is real and promising but meaningfully earlier-stage than Brazil's multi-notebook, multi-session validation. Arabica-only; not validated for Robusta.
- **`core/services/recommendation_engine.py::build_recommendation_from_multiplier()`** — a new shared entry point alongside `build_recommendation()`/`classify_normalized()`, for composite formulas (like India v3) that compute their own upstream multiplier and just need the same threshold/clamp/headline/Recommendation-shape logic applied to it. `generate_india_signal()` (v2, Kodagu-local-climate) is kept, not deleted, and marked superseded in its own docstring — its real output was what made the §6-16 diagnosis possible in the first place.
- 15 new tests across `tests/core/test_recommendation_engine.py` and `tests/domains/coffee/test_signal_generator.py`.

### Why it matters
Both of notebook 06's flagged anomalies had sat as "reported, not resolved" for a full session. Neither is fully resolved even now — that's the honest result, not a shortcoming of this pass — but both are now backed by a specific, executed, falsifiable test rather than a plausible-sounding paragraph: anomaly 1 is confirmed real (its most likely innocent explanation is ruled out) and its actual cause (structural rallies) is directly demonstrated, not just asserted; anomaly 2's mechanism is traced and verified. Separately, India's signal work had been stuck for two sessions on a real, well-diagnosed negative result (§15's FAIL) — productionizing §16's actual positive finding (found in the prior session but never wired into code) turns "we found a promising pattern in a notebook" into "the product can call a real function for it," while keeping the confidence framing honest about how much earlier-stage this validation is than Brazil's.
...
### India §16 — diagnosed §15's FAIL, found a real walk-forward-validated India signal
- **Rather than stopping at §15's FAIL, diagnosed why** — a decisive, useful diagnosis, not just further negative results. Notebook 09 §11: horizon wasn't the issue (Brazil's blended regime is wrong-signed at 3m/6m/12m alike, even more so at 6m). §12: decomposing the blend found the real cause — `(1.5 - price_position)`, Brazil's contrarian term (tuned to global Arabica's own ~24m mean-reversion, notebook 01), correlates *negatively* with India's forward price (r=−0.22 to −0.35), while the climate term alone correlates *positively* and strongly (`climate_amp`, r=+0.548 @ 12m, p<0.0001) — the formula multiplies these, and the wrong-signed term drags the product negative even though most of its ingredients are right-signed for India.
- **§13-§15: re-derived weights on India's own data (not inherited from Brazil) and found a genuinely strong signal.** Leave-one-out ablation: `enso_risk` contributes nothing (delta=0.000, matching its near-zero raw r=+0.028), `cot_momentum` actively hurts (dropping it: +0.043) — both dropped. Best candidate: `price_pos` (momentum-signed, not Brazil's contrarian transform; weight 0.361) + `stu_stress` (0.148) + `l3_risk` (0.491, Brazil's Minas Gerais rainfall as a proxy for *global* Arabica supply risk, since Brazil dominates world production). **Walk-forward validation (weights refit on trailing data only each year, no look-ahead): r=+0.553 (p<0.0001, n=96 months, 8 test years 2017-2024)** — stronger out-of-sample (+0.553) than in-sample (+0.505 full-sample), a good sign against overfitting.
- **Two structural lessons, not incidental tuning:** `price_position` needs a momentum sign for India specifically (opposite of Brazil's mean-reversion-tuned contrarian one — same input, different real-world relationship for a different price series); and a plain additive weighted-sum avoids Brazil's conditional-multiplicative formula's failure mode, where one wrong-signed term can dominate the product's overall sign.
- **Not yet productionized** — no `generate_india_signal()` v3 yet. This is one walk-forward test on ~8 years of India-specific data, real and promising but meaningfully earlier-stage than Brazil's multi-notebook, multi-session validation. Arabica-only (matching §15's own scoping); Robusta not retested. User confirmed proceeding to proper walk-forward validation before treating this as anything more than a screening result. Full writeup in `docs/india_origin_signal_plan_v2_full_build.md` §16 (now v2.7).
