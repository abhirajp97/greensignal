...
- Production wiring for the Brazil composite: `climate_risk_score` weights, `risk_scorer.py`, promote `stu_risk_score` / `enso_lagged` / the SPI drought score into production feature files. `build_recommendation` is no longer a stub (implemented as part of the India origin signal work) but doesn't yet include the rolling-24m normalization the continuous-signal rebuild validated — add that before treating it as final, and confirm which composite formula (of the several under parallel exploration) is being wired first
...
- Walk-forward purchase simulation for notebook 04 (L2a) — the only validated signal still without one.
- Promote notebook 10's Fix 1 (value-blended ML multiplier) into a real composite candidate, or fold it into the notebook 06 composite reconciliation — currently notebook-local, not wired into `signal_generator.py`
...
### Added
- **`notebooks/coffee_backtests/10_ml_composite_model.ipynb`** — new tree-based composite model (XGBoost primary, LightGBM + RandomForest for comparison) combining all five validated signals (L1/L2a/L2b/L3/L5) plus a new realized-volatility feature, trained on 2010-2021 and tested walk-forward on a held-out 2022-2024 window with `TimeSeriesSplit` (5-fold, 2-month embargo) cross-validation. Wrapped in a genuinely new **constrained inventory simulation** (user-configurable monthly usage, storage capacity, budget) — unlike every prior notebook's `cost_improvement_backtest`, which forces total purchase volume to match naive by its own normalization, this tracks a real running inventory bounded by capacity and budget each month.
- `xgboost`, `lightgbm`, `scikit-learn` added as dev dependencies. `libomp` (Homebrew) required locally for XGBoost on macOS — not a new project dependency, a system-level runtime requirement.
- `docs/notebook_10_guide.md` — new reader's guide covering the full pipeline and all three remediation attempts below.

### Findings
- **The ML models have real, statistically significant forecast skill** — XGBoost's predicted `fwd_12m` return correlates with the actual outcome at r=+0.421 (p=0.011) on the held-out test set, LightGBM at +0.329 (p=0.050).
- **But every model LOSES to naive buying in the constrained walk-forward simulation** (−1.3% to −1.6% across two illustrative scenarios), while the legacy hand-specified composite formula (`domains/coffee/sources/*`'s real risk-score functions, run through the identical simulation) BEATS naive (+3.9% to +4.8%).
- **Root cause, diagnosed not just observed:** each model's purchase multiplier correlates POSITIVELY with contemporaneous price (r=+0.40 to +0.45) — it buys more when price is already elevated (momentum), not when price is cheap (value). Feature importances confirm why: `price_pos_52w` — the one feature independently validated to beat naive buying on its own (notebook 01, +7.34%) — ranked last or near-last in every model (0.3-5%), because the model was trained to minimize forward-return forecast error, not to time cheap purchases, and price position has weak `fwd_12m` skill in this data (consistent with notebook 01's own finding that mean-reversion is near-zero at 12m, only emerging at 24m). The same momentum-vs-value mismatch notebook 03 found for the COT signal.
- **Fix 1 (output-transform level) WORKED:** blending the ML forecast into the same value-first skeleton the legacy formula uses (`(1.5 - price_pos) × (1.0 + 0.65 × ml_risk_score)`) flips the momentum correlation to −0.73 and turns the walk-forward loss into a gain (+3.0% to +3.9%, within ~1pp of the legacy formula).
- **Fix 2 (tree-structure level, monotonic constraints) did NOT work:** forcing `price_pos_*` to a non-increasing relationship with the prediction (XGBoost/LightGBM's `monotone_constraints`, scikit-learn 1.4+'s `RandomForestRegressor(monotonic_cst=...)`) barely moved the momentum correlation (+0.34 to +0.40) and left the walk-forward result statistically indistinguishable from the unconstrained model.
- **Fix 3 (tree-structure level, feature-weighted column sampling, XGBoost only) also did NOT work:** a 5x sampling weight on `price_pos_*` columns left the momentum correlation essentially unchanged (r=+0.450) and the walk-forward result slightly worse.
- **Why the tree-structural fixes failed where the output fix worked:** both only increase the *opportunity* or constrain the *direction* for `price_pos` inside the tree — neither forces genuine reliance on it if doing so doesn't reduce training loss, and price position genuinely doesn't reduce `fwd_12m` loss much in this data. A monotonic constraint can be satisfied by making the effect nearly flat everywhere. This is a genuine objective mismatch (predicting price direction vs. timing a cheap purchase), fixable only by constraining the model's output, not its training process.
- A real unit-conversion bug was caught and fixed during development: the simulation initially compared a dollar budget directly against price quoted in USc/lb, collapsing every scenario to an identical 0.00% saving regardless of strategy — fixed with an explicit USc/lb → $/kg conversion.

### Why it matters
This is the first notebook in the project to test whether a learned model can beat the hand-specified composite formula, and the honest answer — twice reconfirmed, once by testing the output transform and twice by testing the training process — is not yet, but not because the model lacks predictive skill: it has real skill, wired to the wrong objective. The remediation attempts turn a plausible-sounding fix ("just make the model use price_pos more") into a concrete, tested result: nudging the tree doesn't work, constraining the output does. `docs/notebook_10_guide.md` documents the reasoning so this doesn't need to be re-derived if the composite is revisited later. Fix 1 is a real candidate worth developing further, but stays notebook-local for now, consistent with how every other validated feature in this project (SPI, true-vintage S/U, COT momentum) has been treated before formal composite wiring.
...
### Changed
- **`notebooks/coffee_backtests/05_chirps_signal.ipynb` — Sec 6 (walk-forward) and Sec 7 (Gate Validation) rebuilt, re-deriving the walk-forward test the [0.25.0] merge had left as a follow-up.** Rather than testing the SPI drought signal standalone (the design discarded at merge), it's now tested as a **binary amplifier on top of the validated L1 price-position signal** (notebook 01, 78w window) — closer to how the signal would actually be used in the product. Gate Validation (old Sec 7) was removed entirely, not just its supporting-robustness content; the correlation gate (Sec 5) stands on its own and the walk-forward is evaluated purely as an economic test.
- **No-look-ahead calibration is now the backbone of the walk-forward, not a side robustness check.** The SPI-3 gamma is refit for every test year using only strictly-prior years (2008-2015 as the calibration baseline, 2016 the first live year) — Sec 3-5's SPI was calibrated on the full 2008-2025 record, a look-ahead violation for a backtest.
- **Binary, not severity-scaled, drought flag** (`SPI-3 <= -1.0`, deliberately in between "any dryness at all" — already shown too weak on its own, r=+0.216 — and a graded weighting scheme, which isn't robustly testable given only 6 historical drought years).
- **Pooled, not per-year, comparison.** A signal that updates once a year produces a purchase weight that's constant within any given calendar year, and a constant weight cancels out of a weighted average exactly — the discarded standalone design's per-year savings were mathematically forced toward ~0% regardless of the signal's quality (a year with a full 1.5x weight boost for 12 straight months still showed ~0% saving). The rebuilt walk-forward instead pools the entire multi-year test window into one weighted-average comparison, which lets between-year weight variation (rather than only within-year variation) show up in the result.
- **Added a sequential (year-by-year) simulation on top of the static pooled comparison**, explicitly modeling how the strategy would look running live: recalibrate once a year (already correct via the no-look-ahead SPI), observe the running cumulative result. Confirms the static and sequential computations agree (+21.34% final row matches the static pooled figure exactly) and surfaces a finding the static number alone doesn't show — see below.

### Findings
- **Pooled 2017-2024 (96 months): L1 alone +20.39%, L1 x SPI drought amp +21.34%, marginal +0.95pp.** Excluding 2023-2024: L1 alone +16.54%, L1 x amp +17.27%, marginal +0.73pp — the marginal effect survives the supercycle-exclusion robustness check, so it isn't purely a 2023-24 artifact.
- **The L1-alone pooled figure (+20.39%) is not comparable to notebook 01's own walk-forward headline (+7.34%)** — different metrics. Notebook 01 averages per-year savings (within-year timing only); this pools the full multi-year span, additionally capturing between-year effects as L1's 78w window drifts across price regimes. Documented explicitly in `docs/notebook_05_guide.md` to head off a misleading comparison.
- **The sequential simulation shows the marginal benefit is concentrated in one episode, not a steady accrual.** Nearly the entire marginal edge appeared in a single jump right after the first flagged year (2017: +2.45pp) — the subsequent flagged years (2020: +0.06pp, 2023: actually *shrinks* the cumulative marginal from +1.74pp to +0.95pp) contributed far less or gave back ground. Of 5 historical drought flags observed live, only one clearly paid off — consistent with the rest of this signal's story (real, mechanistically sound, but thin and single-episode-dependent).
- `docs/notebook_05_guide.md` — recreated (the [0.25.0]-session version was discarded at merge along with the standalone walk-forward it documented). Covers the full SPI rebuild (Sec 0-5, inherited from `main`) and this session's Sec 6 redesign, including both the pooled and sequential results.

### Why it matters
Closes out the "re-derive the walk-forward against the current SPI methodology" item left open at the [0.25.0] merge. More importantly, it demonstrates *why* the redesign was necessary, not just that a number now exists: the original per-year, standalone design was mathematically incapable of detecting this signal's effect regardless of its quality, and the sequential simulation shows that even the corrected pooled design's positive result rests on one well-timed episode rather than a dependable amplifier — the same "real but thin" pattern already established for ENSO and COT's isolated walk-forwards, now extended to CHIRPS/L3.
...
### Added
- **Walk-forward purchase simulations** for notebooks 02 (ENSO) and 03 (COT) — the same design used in notebook 01: naive buying (equal monthly weight) vs. structured buying (purchase weight scaled by the signal, lagged by its own correlation-identified peak), evaluated year by year with no look-ahead.
- `docs/notebook_02_guide.md` — new reader's guide, matching the existing `notebook_01_guide.md` / `notebook_03_guide.md` format (section-by-section explanation, formulas, results, key-numbers table). `docs/notebook_04_guide.md` also added, documenting L2a's pre-vintage-fix state (superseded by the true-vintage rebuild in [0.18.0] below — kept for historical reference, not current).

### Changed
- `notebooks/coffee_backtests/01_ice_price_signal.ipynb` — independently re-executed and confirmed the same result main's own [0.19.0] re-execution found: the **78w window wins, +7.34% avg saving, 10/12 positive years** (52w: +6.05%/9-12; 104w: +6.75%/11-12) — not the "+3.71% avg, 12/12 years" figure that had been miscited elsewhere.
...
- `notebooks/coffee_backtests/03_cot_signal.ipynb` — old "Rolling Stability" section (Sec 7) replaced with a canonical single-weight walk-forward purchase simulation, alongside the notebook's existing multi-variant walk-forward (Sec 5, left untouched) and the new Sec 8 save-CSV cell from [0.19.0] below. Weight rule: `cot_index lagged 6m / 100`. **Result: −2.26% avg saving, 1/12 positive years (2013–2024)** — reinforces, with a simpler design, the notebook's existing conclusion ([0.19.0]) that COT is a low-weight composite amplifier, not a standalone timer.

### Not carried forward
An equivalent walk-forward section was also built for `05_chirps_signal.ipynb` in this same working session, but discarded during merge with `main`: notebook 05 had been independently and substantially rebuilt around SPI ([0.16.0], 2026-07-02) with a different, already-passing gate, so this branch's dryness/`drought_risk_score`-based walk-forward no longer applies. Re-deriving the same walk-forward pattern against the SPI methodology is left as a follow-up (see `[Unreleased]`).

### Why it matters
Two more signals (L2b, L5) now have an economic walk-forward result, not just a correlation gate — and the pattern matches what the composite work ([0.19.0]/[0.20.0]) already assumed: both pass their correlation gates but lose to naive buying standalone. Only L1 clears both a correlation bar and a walk-forward economic bar. Also independently confirms the 78w/+7.34%/10-12 walk-forward figure for L1 that [0.19.0] separately established, closing out a reproducibility question raised earlier in this branch's history.
