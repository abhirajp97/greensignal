...
**All 5 sources implemented, backtested, and now composited (notebook 06) — real-data validation is complete pending one follow-up.**
- **Full composite gate PASSES** (walk-forward ≥3.50%) on both weight schemes tested: Phase 0 fixed weights (0.38/0.24/0.22/0.16) → **+9.31%**; real-data r-proportional reweighting (each sub-signal's own weight ∝ its |r| against forward return, measured within the aligned composite frame) → **+3.62%**. Phase 0 synthetic baseline was 4.54%; full-history in-sample screening (before walk-forward) was more moderate — Phase 0 +4.44%, r-proportional +4.73%.
- **Caveat that matters more than the PASS itself: thin walk-forward sample.** Only 11 (Phase 0) / 21 (r-proportional) buy-months total across the 10 walk-forward test years (2015–2024), with 3–4 of those years classifying *zero* buy months. Both numbers clear the numeric gate but shouldn't be read as a confident, stable result — treat this as directional validation of the composite approach, not a final precise weight assignment.
- **A joint multivariate regression was deliberately not used as the primary weighting method** — 4 free parameters fit on a sample that includes two borderline signals (L3 p=0.069 n=15, L5 p=0.053) risks fitting noise, and a fitted coefficient doesn't translate to plain-language product copy the way a hand-set weight does. A regularized (Ridge, non-negative) ceiling check was run for comparison — it did **not** beat the simpler r-proportional scheme in-sample, and notably zeroed out L5's coefficient entirely under its own constraint, a real disagreement with the ablation finding below.
- **Leave-one-out ablation (generalized to all 4 sub-signals — L3 and L5 aren't the only ones worth scrutinizing), full-history in-sample:** dropping **L2b costs −3.00pp** (by far the strongest individual contributor — its highest weight is earned); dropping **L5 costs −0.81pp** (helps — empirically validates notebook 03's "low-weight momentum amplifier" conclusion rather than assuming it); dropping **L3 costs only −0.22pp** (near-neutral, consistent with its "confirming amplifier" framing, not a timing signal); dropping **L2a *improves* the number by +1.06pp** — a genuinely surprising result traced to a specific, identified cause (not smoothed over): the `stu_stress` input used in notebook 06 was built off the PSD-**approximation** series (nb04's 12m-shift fallback), not the stronger **true-vintage** series (nb04's Gate 1 r=−0.488 vs the approximation's −0.312). **Before finalizing L2a's composite weight, rebuild its risk score off the true-vintage series** (n=30 semiannual, needs its own small-n windowing) and re-run the ablation — don't conclude L2a should be dropped from this result alone.
- Spike avoidance (2024 Arabica +110%): 200 kg/month roaster saved ~$3,000 in one quarter — not directly re-tested in notebook 06 (focus was the prescience gate + ablation).
...
4. ✅ Composite backtest (notebook 06) on real data — gate PASSES on both weight schemes (thin walk-forward sample, see caveat above); ablation validates L5's momentum-amplifier role and L2b as the strongest contributor; L2a's stress-score input needs rebuilding off the true-vintage series before final weights
5. ← **NEXT:** Rebuild `stu_stress` off the true-vintage series and re-run the L2a ablation, then production wiring: `climate_features.py::climate_risk_score`'s weights (currently hardcoded Phase 0), `domains/coffee/models/risk_scorer.py` (stub), promote `stu_risk_score`/`enso_lagged`/the SPI drought score into production feature files. `core/services/recommendation_engine.py::build_recommendation` is no longer a stub — implemented as part of the India origin signal work below (origin-agnostic, used by both Brazil's `generate_signal()` and India's `generate_india_signal()`). **This step also unblocks a deferred India validation**: once this composite produces real historical recommendations, its forecasting transferability to India (translating through the §14 pass-through equation, not just the contemporaneous fit already proven) can finally be tested — see `docs/india_origin_signal_plan_v2_full_build.md` §15 and `docs/NEXT_STEPS.md`
