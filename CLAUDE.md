...
- **Primary gate switched from the discrete prescience test to the continuous `cost_improvement_backtest` methodology** (`docs/GreenSignal_Math_Reference.md` §11.1 — the same one notebook 01 used to validate L1 alone), since that's what Phase 0's own doc treats as the primary ROI story and what actually mirrors the product's continuous purchase-volume-scaling design. **PASSES: walk-forward +5.86%** — full-history is +4.45% (vs. L1-alone's +10.73%). (L1-alone's own walk-forward benchmark is **+7.34%**, 78w window — corrected from an earlier unreproducible "+3.71%" figure; see "Walk-Forward Economic Validation" below. The composite's +5.86% no longer clearly beats L1 alone on this corrected number, though the two aren't a strict apples-to-apples comparison — L1-alone's figure is 2013-2024/12yr, the composite's is the 2015-2024/10yr walk-forward window.)
...
**The correlation gate result does not predict the walk-forward result.** Both L2b and L5 pass their correlation gates but lose to naive buying standalone. Only L1 clears both a correlation bar and a walk-forward economic bar. This is real evidence — not just the pre-existing qualitative judgment — that L2b/L5 belong in the composite as amplifiers on top of L1 (per the conditional formula above), not as standalone timing signals. L2a (USDA stocks-to-use) does not yet have a walk-forward section; given its much stronger price-level correlation it's a reasonable hypothesis it would fare better, but that's untested.

**L3 (CHIRPS SPI) is tested differently — as an amplifier on L1, not isolated —** since that's how it's meant to be used in the composite. `weight = buy_signal_78w × (1 + drought_flag)`, where `drought_flag` is a binary no-look-ahead SPI-3 flag (gamma refit per year on strictly-prior years only; 2008-15 calibration baseline, live from 2016; SPI-3 ≤ −1.0 threshold — binary rather than severity-scaled, since magnitude carries real information but rests on only 6 historical drought years, too thin to validate a graded weighting scheme). Pooled 2017-2024 (96 months, mathematically necessary since a signal this update-frequency-limited produces a within-year-constant weight that cancels out of any per-year comparison): **L1 alone +20.39%, L1×amp +21.34%, marginal +0.95pp** (+0.73pp excl. 2023-24 — not purely a supercycle artifact). *Not comparable to L1's own +7.34% row above* — that's a per-year-averaged metric, this is a full-period pool, which additionally captures between-year effects. A sequential year-by-year simulation shows the marginal edge is concentrated almost entirely in one well-timed flag (2017), not a steady accrual — see `CHANGELOG.md [0.26.0]` and `docs/notebook_05_guide.md`.
