...
**All 5 sources now implemented and backtested on real data — all 5 clear their gates.** Composite (notebook 06) is the remaining validation:
- Full 5-signal composite: **4.54% forward prescience** (Phase 0 synthetic) — must be re-run now that L1, **L2a (true vintage, dual gate)**, **L2b (on the ~14m El Niño lead)**, **L3 (SPI flowering deficit, confirming)**, and **L5 (momentum, r=+0.144 @ 6m, weak/borderline)** all clear their gates. Reweighting needed — L5's weak rolling stability (45%) and L3's borderline significance (p=0.069, n=15) both merit a leave-one-out ablation to confirm they're adding real forward-prescience value, not just noise.
...
4. ← **NEXT:** Rebuild Phase 0 composite backtest (notebook 06) on real data — confirm rank order and reweight. All 5 signals clear their gates: L1 (strong), L2a (true vintage, dual gate), L2b (~14m El Niño lead), L3 (SPI flowering deficit, confirming, borderline p=0.069), L5 (momentum, weak/borderline, 45% rolling stability). Leave-one-out ablation on L3/L5 before finalizing weights
