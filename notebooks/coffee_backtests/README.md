...
| L2a — stocks-to-use % | **Redefined, vintage-lagged, dual gate:** Gate 1 r(vintage S/U, 12m-fwd price level) / Gate 2 r(S/U YoY delta, price level) | −0.35 | Gate 1 ≤ −0.25 / Gate 2 ≤ −0.20 | ✅ PASS both — Gate 1 r=−0.312 (p=2.0e-5, n=180), Gate 2 r=−0.259 (p=4.6e-4, n=180). **Vintage rebuild:** the PSD bulk file is the latest-revised vintage, so the series is shifted 12m forward to simulate the real publication lag before correlating (raw r=−0.40 weakens to −0.26 once corrected — the honest number). Old single YoY-change gate (r=−0.04) FAILED — wrong lens for an annual stock var |
